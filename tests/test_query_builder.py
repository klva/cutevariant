...
def test_get_table_of_column(conn):
    ...
    assert (
        sql.QueryBuilder(conn).get_table_of_column("annotations.gene") == "annotations"
    )
    ...
    assert (
        sql.QueryBuilder(conn).get_table_of_column(("genotype", "TUMOR", "af"))
        == "samples"
    )
...
def test_filters_to_sql(conn):
    ...
    assert (
        sql.QueryBuilder(conn)._filters_to_sql(FILTERS)
        == "(`variants`.`chr` = 'chr1' AND (`annotations`.`gene` = 'gene1' OR `variants`.`pos` = 10))"
    )
...
def test_select_children(conn):
    assert len(list(sql.QueryBuilder(conn).children(variant_id=1))) == len(
        VARIANTS[0]["annotations"]
    )
    assert len(list(sql.QueryBuilder(conn).children(variant_id=2))) == len(
        VARIANTS[1]["annotations"]
    )
...
@pytest.mark.parametrize(
    "args,expected",
    [
        ({"columns": ["chr", "pos"]}, "SELECT chr,pos FROM variants"),
        (
            {"columns": ["chr", "pos", ("genotype", "boby", "gt")]},
...
def test_column_to_sql(conn):
    ...
    assert (
        selector.column_to_sql(("genotype", "boby", "gt"))
        == "`gt_boby`.`gt` AS `gt_boby.gt`"
    )
