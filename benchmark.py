...
def generate_variant(count=10):
    for i in range(count):
        yield {
            ...
            "pos": random.randint(1000, 100_000),
...
schema = ",".join(
    ["variant_id INT"] + [f"field{j} TEXT" for j in range(ANNOTAION_FIELD_COUNT)]
)
...
for variant in generate_variant(VARIANT_COUNT):
    ...
    for annotation in variant["annotations"]:
        ...
        schema = ",".join(
            [":variant_id"] + [f":field{j}" for j in range(ANNOTAION_FIELD_COUNT)]
        )
...
start = time.time()
cursor.execute(
    "SELECT * FROM variants LEFT JOIN annotations ON annotations.variant_id = variants.rowid LIMIT 0,100"
)
end = time.time()
